...
def get_ss_sku(template_for_ss_sku, budj_AMD, ost_AMP_d, ost_AMD, mapping, month, year_report,df_list_ss_sku_itog, global_index):
    ...
    for ind___, current_month in enumerate(iter_months):
        ...
        # ---------------------------------------------------
        curr_cost_amd['map2'] = curr_cost_amd['map2'].astype('category')
        curr_cost_amd['map3'] = curr_cost_amd['map3'].astype('category')
        stock_sebes['канал'] = stock_sebes['канал'].astype('category')
        def func(x, df):
